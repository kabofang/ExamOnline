...
CXmlNodeWrapper::~CXmlNodeWrapper()
{
	if (!m_bAutoRelease)
		m_xmlnode.Detach();
	/*MSXML2::IXMLDOMNode *pNode = (MSXML2::IXMLDOMNode *)m_xmlnode.Detach();
	if (pNode)
	{
		(MSXML2::IXMLDOMNode *)pNode->Release();
		(MSXML2::IXMLDOMNode *)pNode->Release();
	}*/
}
...
MSXML2::IXMLDOMNodePtr CXmlNodeWrapper::InsertNode(int index,LPCTSTR nodeName)
{
	...
	if (xmlDocument)
	{
		...
		if (refNode != NULL)
			newNode = m_xmlnode->insertBefore(newNode,refNode);
		else
...
MSXML2::IXMLDOMNodePtr CXmlNodeWrapper::InsertBefore(MSXML2::IXMLDOMNode* refNode, LPCTSTR nodeName)
{
	MSXML2::IXMLDOMDocumentPtr xmlDocument = m_xmlnode->GetownerDocument();
	if (xmlDocument)
	{
		MSXML2::IXMLDOMNodePtr newNode = xmlDocument->createNode(_variant_t((short)MSXML2::NODE_ELEMENT),nodeName,"");
		newNode = m_xmlnode->insertBefore(newNode,(IUnknown*)refNode);
		return newNode;
	}
	return NULL;
}

MSXML2::IXMLDOMNodePtr CXmlNodeWrapper::InsertAfter(MSXML2::IXMLDOMNode *refNode, LPCTSTR nodeName)
{
	MSXML2::IXMLDOMDocumentPtr xmlDocument = m_xmlnode->GetownerDocument();
	if (xmlDocument)
	{
		...
		if (nextNode.GetInterfacePtr() != NULL)
			newNode = m_xmlnode->insertBefore(newNode,(IUnknown*)nextNode);
		else
			newNode = m_xmlnode->appendChild(newNode);
		return newNode;
	}
...
MSXML2::IXMLDOMNodePtr CXmlNodeWrapper::InsertAfter(MSXML2::IXMLDOMNode *refNode, MSXML2::IXMLDOMNode *pNode)
{
	MSXML2::IXMLDOMNodePtr nextNode = refNode->GetnextSibling();
...
XMLOperation::XMLOperation(LPCTSTR XMLFilePath)
{
	m_validXML = false;

	m_XMLFileName = XMLFilePath;

	m_xmldoc.CreateInstance(MSXML2::CLSID_DOMDocument);

	if((m_xmldoc == NULL)||(m_xmldoc.GetInterfacePtr() == NULL))
	{
		m_validXML = false;
	}
	else
	{
		_variant_t v(XMLFilePath);
		m_xmldoc->put_async(VARIANT_FALSE);
		VARIANT_BOOL success = m_xmldoc->load(v);
		if (success == VARIANT_TRUE)
		{
...
bool XMLOperation::IsValidXMLFile()
{
	return m_validXML;
}


CString XMLOperation::SearchNodeText(IDispatch *pNode, LPCTSTR pNodeName, LPCTSTR pAttributeName)
{
	...
	if(node.NodeType() == "element")
	{
		...
		if(str == pNodeName)
		{
			if (pAttributeName)
			{
				return node.GetValue(pAttributeName);
			} 
			else
			{
				return node.GetText();
	...
	{
		res = SearchNodeText(node.GetNode(i),pNodeName,pAttributeName);
...
CString XMLOperation::ReadXMLPolicy(LPCTSTR pNodeName,
									LPCTSTR pAttributeName)
{
	...
	return SearchNodeText(m_xmldoc->GetdocumentElement(),pNodeName,pAttributeName);
}
